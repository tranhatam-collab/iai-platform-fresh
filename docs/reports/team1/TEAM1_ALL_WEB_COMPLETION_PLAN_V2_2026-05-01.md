...
| D8 | Cloudflare source-of-truth reconciliation, split by wave (each sub-scope is independently deploy-blocking for its wave): D8a `home-iai-one` is linked to legacy Next.js lineage while monorepo `apps/home` is Node TS — **W1A** — **PAUSED 2026-05-02** pending founder architecture decision (`NEXT_VS_NODE_DRIFT_2026-05-02.md`). D8b `docs-iai-one` is linked to legacy Pages source while monorepo `apps/docs` is Node TS — **W1B** — **HELD 2026-05-02** pending docs runtime-model decision (`TEAM1_W1B_D8B_EXECUTION_PACKET_2026-05-02.md`). D8c `flow-iai-one` + `iai-dash` currently have deployment sources not traceable to monorepo commits — **W2** — **confirmed open**. D8d `app-iai-one` is live Next.js lineage while monorepo `apps/app` is Node TS — **W4** — **confirmed open**. | `AUDIT_LIVE_INFRASTRUCTURE_2026-05-02.md` §B1; `NEXT_VS_NODE_DRIFT_2026-05-02.md`; `AUDIT_RUNTIME_SOURCE_DRIFT_MATRIX_2026-05-02.md` | Founder / Team 5 Web / Team Runtime | D8a → W1A, D8b → W1B, D8c → W2, D8d → W4 (each gates its own wave deploy) | open (D8a paused, D8b held, D8c/D8d confirmed-open) |
...
2. **Preview / production deploy approval gate.** No preview or production deploy of a wave may be triggered while ANY `closes at` item for that wave remains `open`. This is what actually controls live infrastructure. Specifically:
   - **W1A preview deploy is blocked** until D8a (`home-iai-one` Pages source reconciled to monorepo) is closed. D7 (GitHub remote + push) was closed on 2026-05-02; see `TEAM1_W1A_D7_CLOSEOUT_2026-05-02.md`. D12 (`root.iai.one` DNS decision) was closed on 2026-05-02 via path B; see `TEAM1_W1A_D12_CLOSEOUT_2026-05-02.md`.
...
- D8a remains open and is **paused** as of 2026-05-02. The drift report `NEXT_VS_NODE_DRIFT_2026-05-02.md` confirmed that live `home.iai.one`/`iai.one` are Next.js while monorepo `apps/home`/`apps/root` are Node TS servers; this is a two-codebase technology drift, not a simple Pages source switch. The execution packet `TEAM1_W1A_D8A_EXECUTION_PACKET_2026-05-02.md` is held and must NOT be executed before founder explicitly picks an architecture target (Next.js canonical vs Node canonical) per drift report §4.
- D8b is now confirmed-open and **held** as of 2026-05-02. `TEAM1_W1B_D8B_EXECUTION_PACKET_2026-05-02.md` records that live `docs.iai.one` source lineage is not traceable to monorepo while monorepo `apps/docs` is Node TS runtime lane. Founder must choose docs runtime model before D8b may close.
