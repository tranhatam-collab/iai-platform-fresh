...
9. On 2026-05-02, after the live infrastructure audit (`AUDIT_LIVE_INFRASTRUCTURE_2026-05-02.md`), six new deferred items D7–D12 were added to §7. D7 (GitHub push) is closed in `TEAM1_W1A_D7_CLOSEOUT_2026-05-02.md`. D12 (`root.iai.one` DNS/trust-state decision) is closed via path B in `TEAM1_W1A_D12_CLOSEOUT_2026-05-02.md`. D8 (Cloudflare Pages source reconciliation) remains deploy-blocking — D8a is the only remaining W1A preview-deploy blocker. **Update 2026-05-02:** while preparing D8a closeout, a two-codebase technology drift was confirmed (live `home.iai.one` and `iai.one` are Next.js; monorepo `apps/home` and `apps/root` are Node TypeScript servers). D8a is therefore **paused** pending an explicit founder architecture decision; see `NEXT_VS_NODE_DRIFT_2026-05-02.md`. The D8a execution packet (`TEAM1_W1A_D8A_EXECUTION_PACKET_2026-05-02.md`) must NOT be executed until that decision is made. D2 and D3 are marked closed in §7 referencing `TEAM1_W1A_D2_D3_CLOSEOUT_2026-05-02.md`.
10. On 2026-05-02 (later), runtime/source drift was also confirmed across `docs/app/flow/dash` in `AUDIT_RUNTIME_SOURCE_DRIFT_MATRIX_2026-05-02.md`. D8b is no longer "unverified"; it is now confirmed-open drift with a held execution packet (`TEAM1_W1B_D8B_EXECUTION_PACKET_2026-05-02.md`). D8c and D8d are similarly confirmed-open by evidence, so W2/W4 should treat them as known reconciliation work, not surprise work.
...
5. founder triggers `W1B` preview deploy of legacy `docs-iai-one` Pages project via Cloudflare dashboard (and pastes binding repo URL into `docs/SURFACE_SOURCE_OF_TRUTH.md` §1).
...
2. **Preview / production deploy approval gate.** No preview or production deploy of a wave may be triggered while ANY `closes at` item for that wave remains `open`. This is what actually controls live infrastructure. Specifically:
   - **W1A preview deploy is unblocked** as of 2026-05-02: D7 closed via `TEAM1_W1A_D7_CLOSEOUT_2026-05-02.md`, D12 closed via path B in `TEAM1_W1A_D12_CLOSEOUT_2026-05-02.md`, D8a closed via path B1 in `TEAM1_W1A_D8A_CLOSEOUT_PATH_B1_2026-05-02.md`. The actual preview build runs on legacy `tranhatam-collab/Home.iai.one` via Cloudflare Pages dashboard (founder action). **W1B preview deploy is also unblocked** as of 2026-05-02 via `TEAM1_W1B_D8B_CLOSEOUT_PATH_P1_2026-05-02.md`.
...
- D8b was **closed on 2026-05-02 via path P1** following founder reply D-003 = `PAGES`. Closeout: `TEAM1_W1B_D8B_CLOSEOUT_PATH_P1_2026-05-02.md`. The legacy Pages binding for `docs-iai-one` is now declared canonical; monorepo `apps/docs` is marked `experimental, not_live` via `apps/docs/STATUS.md`. The exact GitHub repo bound to `docs-iai-one` cannot be read via wrangler CLI; capture procedure is logged in `docs/reports/team1/artifacts/d8b/DOCS_IAI_ONE_BINDING_CAPTURE_NOTE_2026-05-02.md`.
