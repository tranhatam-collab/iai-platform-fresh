# IAI_ONE_W1A_PRE_DEPLOY_EVIDENCE_PACKET_2026-05-01

- Date: `2026-05-01`
- Surface: `iai.one`
- Wave: `W1A`
- Status: `READY_FOR_W1A_PREVIEW_DEPLOY`
...
Prepare `iai.one` for founder review before preview/prod deploy.

This packet covers repo-side truth cleanup only:

- keep `iai.one` truthful while `web.iai.one` has no accepted DNS/deploy/owner proof
...
- keep bilingual root shell and canonical metadata intact
...
|---|---|---:|---|---|
| `/` | renders constitutional root shell in VI/EN, no default `web.iai.one` link | no | PASS | `pnpm test:root` |
| `/health` | returns `service=iai-root`, `web_surface_enabled=false`, `web_url=null` | no | PASS | `pnpm test:root` |
| `/og.svg` | returns localized SVG social image | no | covered by existing root route implementation | code review |
| `/missing` | returns explicit 404 shell | no | PASS | `pnpm test:root` |

## 4. QC Evidence

| Check | Result | Notes |
|---|---:|---|
| `pnpm typecheck:root` | PASS | Included in `pnpm test:root` build step |
| `pnpm test:root` | PASS `5/5` | Root shell, health, EN/VI, explicit re-enable flag, 404 |
...
| Public navigation truth | PASS | Default root HTML does not expose `https://web.iai.one` |
| Health truth | PASS | `web_surface_enabled=false`, `web_url=null` |
| D2 legal URL | PASS | Root footer exposes `https://docs.iai.one/legal/iai-flow/` |
| D3 legal entity | PASS | Root footer exposes `Angel Edu Tam Foundation Inc` |
...
- change is a release-hold truth cleanup, not a runtime expansion
- public links are reduced, not expanded
- explicit re-enable path remains available via `ROOT_WEB_SURFACE_ENABLED=true`
...
Plan V2 packet spec files are present under `docs/release-evidence/iai.one/`:
...
1. trigger preview deploy for canonical legacy repo `tranhatam-collab/Home.iai.one` via Cloudflare Pages dashboard. D7, D12, and D8a are already closed.
2. attach preview evidence for `iai.one`
