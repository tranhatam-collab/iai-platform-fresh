...
import { createFlowServer } from "../apps/flow/dist/server.js";
...
const outputDir = "docs/release-evidence/flows.iai.one/artifacts";
const markdownPath = `${outputDir}/FLOWS_IAI_ONE_LOCAL_ROUTE_PROOF_${dateTag}.md`;
const jsonPath = `${outputDir}/FLOWS_IAI_ONE_LOCAL_ROUTE_PROOF_${dateTag}.json`;

const routes = ["/", "/health", "/missing"];
...
if (!address || typeof address === "string") {
  server.close();
  throw new Error("Unable to resolve local flow server address.");
}
...
try {
  ...
  const englishCheck = await checkRoute(baseUrl, "/?lang=en");
  checks.push({ ...englishCheck, route: "/?lang=en" });
} finally {
  await new Promise((resolve) => {
...
function renderMarkdown(baseUrlValue, generatedAtValue, checksValue) {
  const lines = [];
  lines.push(`# Flows Local Route Proof ${dateTag}`);
  lines.push("");
