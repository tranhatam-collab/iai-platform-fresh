...
const paySeoEntry = loadSeoEntry("pay.iai.one");
...
export function buildLocalizedPath(path: string, locale: Locale): string {
  const url = new URL(path, paySeoEntry.canonical);
...
export function getPageMetadata(path: string, locale: Locale, pageTitle?: string) {
  const seoTitle = locale === "vi" ? paySeoEntry.titleVi : paySeoEntry.titleEn;
  const description = locale === "vi" ? paySeoEntry.descriptionVi : paySeoEntry.descriptionEn;
  const title = path === "/" && !pageTitle ? seoTitle : `${pageTitle ?? seoTitle} | ${seoTitle}`;
  return {
    alternates: {
      en: new URL(buildLocalizedPath(path, "en"), paySeoEntry.canonical).toString(),
      vi: new URL(buildLocalizedPath(path, "vi"), paySeoEntry.canonical).toString(),
      xDefault: new URL(buildLocalizedPath(path, defaultLocale), paySeoEntry.canonical).toString()
    },
    canonical: new URL(buildLocalizedPath(path, locale), paySeoEntry.canonical).toString(),
    description,
    ...
    schemaTypes: paySeoEntry.schemaTypes,
    title
  };
}
