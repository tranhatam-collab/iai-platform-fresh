...
const developerSeoEntry = loadSeoEntry("developer.iai.one");
...
export function buildLocalizedPath(path: string, locale: Locale): string {
  const url = new URL(path, developerSeoEntry.canonical);
...
export function getPageMetadata(path: string, locale: Locale, pageTitle?: string) {
  const seoTitle = locale === "vi" ? developerSeoEntry.titleVi : developerSeoEntry.titleEn;
  const description = locale === "vi" ? developerSeoEntry.descriptionVi : developerSeoEntry.descriptionEn;
  const title = path === "/" && !pageTitle ? seoTitle : `${pageTitle ?? seoTitle} | ${seoTitle}`;
  return {
    alternates: {
      en: new URL(buildLocalizedPath(path, "en"), developerSeoEntry.canonical).toString(),
      vi: new URL(buildLocalizedPath(path, "vi"), developerSeoEntry.canonical).toString(),
      xDefault: new URL(
        buildLocalizedPath(path, defaultLocale),
        developerSeoEntry.canonical
      ).toString()
    },
    canonical: new URL(buildLocalizedPath(path, locale), developerSeoEntry.canonical).toString(),
    description,
    htmlLang: localeMeta[locale].htmlLang,
    socialImage: buildSurfaceSocialImageUrl({
      description,
      locale,
      surface: "developer.iai.one",
      title
    }),
    schemaTypes: developerSeoEntry.schemaTypes,
    title
  };
}
