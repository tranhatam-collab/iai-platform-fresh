...
const dashSeoEntry = loadSeoEntry("dash.iai.one");
...
export function buildLocalizedPath(path: string, locale: Locale): string {
  const url = new URL(path, dashSeoEntry.canonical);
...
export function getPageMetadata(path: string, locale: Locale, pageTitle: string) {
  const canonical = new URL(buildLocalizedPath(path, locale), dashSeoEntry.canonical).toString();
  const description = locale === "vi" ? dashSeoEntry.descriptionVi : dashSeoEntry.descriptionEn;
  const title = `${pageTitle} | ${locale === "vi" ? dashSeoEntry.titleVi : dashSeoEntry.titleEn}`;
  return {
    alternates: {
      en: new URL(buildLocalizedPath(path, "en"), dashSeoEntry.canonical).toString(),
      vi: new URL(buildLocalizedPath(path, "vi"), dashSeoEntry.canonical).toString(),
      xDefault: new URL(buildLocalizedPath(path, defaultLocale), dashSeoEntry.canonical).toString()
    },
    canonical,
    description,
    ...
    socialImage: buildSurfaceSocialImageUrl({
      description,
      locale,
      surface: "dash.iai.one",
      title
    }),
    schemaType: dashSeoEntry.schemaType,
    title
  };
}
