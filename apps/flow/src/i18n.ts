...
const flowSeoEntry = loadSeoEntry("flow.iai.one");
...
export function buildLocalizedPath(path: string, locale: Locale): string {
  const url = new URL(path, flowSeoEntry.canonical);
...
export function getPageMetadata(path: string, locale: Locale, pageTitle?: string) {
  const seoTitle = locale === "vi" ? flowSeoEntry.titleVi : flowSeoEntry.titleEn;
  const description = locale === "vi" ? flowSeoEntry.descriptionVi : flowSeoEntry.descriptionEn;
  const title = path === "/" && !pageTitle ? seoTitle : `${pageTitle ?? seoTitle} | ${seoTitle}`;
  return {
    alternates: {
      en: new URL(buildLocalizedPath(path, "en"), flowSeoEntry.canonical).toString(),
      vi: new URL(buildLocalizedPath(path, "vi"), flowSeoEntry.canonical).toString(),
      xDefault: new URL(buildLocalizedPath(path, defaultLocale), flowSeoEntry.canonical).toString()
    },
    canonical: new URL(buildLocalizedPath(path, locale), flowSeoEntry.canonical).toString(),
    description,
    htmlLang: localeMeta[locale].htmlLang,
    socialImage: buildSurfaceSocialImageUrl({
      description,
      locale,
      surface: "flow.iai.one",
      title
    }),
    schemaTypes: flowSeoEntry.schemaTypes,
    title
  };
}
