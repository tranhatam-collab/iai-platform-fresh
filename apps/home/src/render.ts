...
export interface HomeRenderConfig {
  appUrl: string;
  dashUrl: string;
  developerUrl: string;
  docsUrl: string;
...
export function renderHomePortal(config: HomeRenderConfig, locale: Locale): string {
  return page(
    "/",
    locale,
    undefined,
    `
      <header class="topbar">
        <div class="brand">
          <p class="eyebrow">${escapeHtml(t(locale, "surface.home.domain"))}</p>
          <strong>${escapeHtml(t(locale, "surface.portal.title"))}</strong>
        </div>
      ...
      <main class="page-shell">
        <section class="hero">
          <div class="hero-copy">
            <p class="eyebrow">${escapeHtml(t(locale, "home.hero.eyebrow"))}</p>
            <h1>${escapeHtml(t(locale, "surface.portal.title"))}</h1>
            <p class="lede">${escapeHtml(t(locale, "home.hero.body"))}</p>
            <p class="note">${escapeHtml(t(locale, "home.hero.note"))}</p>
            <div class="actions">
              <a class="primary" href="${escapeHtml(localizeExternalUrl(config.appUrl, locale))}">${escapeHtml(
          ...
          <aside class="panel boundary-panel">
            <p class="eyebrow">${escapeHtml(t(locale, "home.boundaries.eyebrow"))}</p>
            <h2>${escapeHtml(t(locale, "home.boundaries.title"))}</h2>
            <p>${escapeHtml(t(locale, "home.boundaries.body"))}</p>
          </aside>
        </section>

        <section class="section-head">
          <p class="eyebrow">${escapeHtml(t(locale, "home.paths.eyebrow"))}</p>
          <h2>${escapeHtml(t(locale, "home.paths.title"))}</h2>
          <p>${escapeHtml(t(locale, "home.paths.body"))}</p>
        </section>

        <section class="surface-grid">
          ${renderSurfaceCard(locale, t(locale, "surface.root.title"), t(locale, "home.surface.root.body"), config.rootUrl, t(locale, "btn.read_boundaries"))}
          ${renderSurfaceCard(locale, t(locale, "surface.app.title"), t(locale, "home.surface.app.body"), config.appUrl, t(locale, "btn.open"))}
          ${renderSurfaceCard(locale, t(locale, "surface.flow.title"), t(locale, "home.surface.flow.body"), config.flowUrl, t(locale, "btn.open_runtime"))}
          ${renderSurfaceCard(locale, t(locale, "surface.docs.title"), t(locale, "home.surface.docs.body"), config.docsUrl, t(locale, "btn.open_docs"))}
        ...
        <section class="boundary-grid">
          <article class="boundary-card">
            <p>${escapeHtml(t(locale, "home.boundary1"))}</p>
          </article>
          <article class="boundary-card">
            <p>${escapeHtml(t(locale, "home.boundary2"))}</p>
          </article>
          <article class="boundary-card">
            <p>${escapeHtml(t(locale, "home.boundary3"))}</p>
          </article>
        </section>
      </main>
...
export function renderHomeNotFound(locale: Locale, path: string): string {
  return page(
    path,
    locale,
    t(locale, "home.page.not_found"),
    `
      <header class="topbar">
        <div class="brand">
          <p class="eyebrow">${escapeHtml(t(locale, "surface.home.domain"))}</p>
      ...
      <main class="page-shell">
        <section class="hero hero-simple">
          <div class="hero-copy">
            <p class="eyebrow">${escapeHtml(t(locale, "surface.home.domain"))}</p>
            <h1>${escapeHtml(t(locale, "home.not_found.title"))}</h1>
            <p class="lede">${escapeHtml(t(locale, "home.not_found.body"))}</p>
            <div class="actions">
              <a class="primary" href="${escapeHtml(buildLocalizedPath("/", locale))}">${escapeHtml(
                t(locale, "home.not_found.back")
              )}</a>
            </div>
          </div>
        </section>
  ...
  <head>
    ...
    <style>
      :root {
        ...
        --accent: #245d6b;
        --accent-soft: rgba(36, 93, 107, 0.1);
        --accent-strong: #173840;
      }
      ...
      * { box-sizing: border-box; }
      ...
      body {
        ...
        background:
          radial-gradient(circle at top right, rgba(36, 93, 107, 0.12), transparent 26%),
          radial-gradient(circle at top left, rgba(23, 56, 64, 0.08), transparent 22%),
          linear-gradient(180deg, #ece5d9 0%, var(--paper) 45%, #f7f1e8 100%);
      }
