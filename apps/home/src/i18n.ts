...
const homeSeoEntry = loadSeoEntry("home.iai.one");
...
export function buildLocalizedPath(path: string, locale: Locale): string {
  const url = new URL(path, homeSeoEntry.canonical);
...
export function getPageMetadata(path: string, locale: Locale, pageTitle?: string) {
  const seoTitle = locale === "vi" ? homeSeoEntry.titleVi : homeSeoEntry.titleEn;
  const description = locale === "vi" ? homeSeoEntry.descriptionVi : homeSeoEntry.descriptionEn;
  const title = path === "/" && !pageTitle ? seoTitle : `${pageTitle ?? seoTitle} | ${seoTitle}`;
  return {
    alternates: {
      en: new URL(buildLocalizedPath(path, "en"), homeSeoEntry.canonical).toString(),
      vi: new URL(buildLocalizedPath(path, "vi"), homeSeoEntry.canonical).toString(),
      xDefault: new URL(buildLocalizedPath(path, defaultLocale), homeSeoEntry.canonical).toString()
    },
    canonical: new URL(buildLocalizedPath(path, locale), homeSeoEntry.canonical).toString(),
    description,
    ...
    schemaTypes: homeSeoEntry.schemaTypes,
    title
  };
}
