...
- Reason: Founder picked Next.js canonical (D-002 = `B`) on 2026-05-02. Live `home.iai.one` is served from legacy Next.js repo `tranhatam-collab/Home.iai.one` via Cloudflare Pages project `home-iai-one`. This Node TS server is **not** the live source.
