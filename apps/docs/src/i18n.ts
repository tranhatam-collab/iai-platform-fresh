...
const docsSeoEntry = loadSeoEntry("docs.iai.one");
...
export function buildLocalizedPath(path: string, locale: Locale): string {
  const url = new URL(path, docsSeoEntry.canonical);
...
export function getPageMetadata(path: string, locale: Locale, pageTitle?: string) {
  const seoTitle = locale === "vi" ? docsSeoEntry.titleVi : docsSeoEntry.titleEn;
  const description = locale === "vi" ? docsSeoEntry.descriptionVi : docsSeoEntry.descriptionEn;
  const title = path === "/" && !pageTitle ? seoTitle : `${pageTitle ?? seoTitle} | ${seoTitle}`;
  return {
    alternates: {
      en: new URL(buildLocalizedPath(path, "en"), docsSeoEntry.canonical).toString(),
      vi: new URL(buildLocalizedPath(path, "vi"), docsSeoEntry.canonical).toString(),
      xDefault: new URL(buildLocalizedPath(path, defaultLocale), docsSeoEntry.canonical).toString()
    },
    canonical: new URL(buildLocalizedPath(path, locale), docsSeoEntry.canonical).toString(),
    description,
    ...
    schemaTypes: docsSeoEntry.schemaTypes,
    title
  };
}
