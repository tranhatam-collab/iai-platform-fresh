...
const nftSeoEntry = loadSeoEntry("nft.iai.one");
...
export function buildLocalizedPath(path: string, locale: Locale): string {
  const url = new URL(path, nftSeoEntry.canonical);
...
export function getPageMetadata(path: string, locale: Locale, pageTitle?: string) {
  const seoTitle = locale === "vi" ? nftSeoEntry.titleVi : nftSeoEntry.titleEn;
  const description = locale === "vi" ? nftSeoEntry.descriptionVi : nftSeoEntry.descriptionEn;
  const title = path === "/" && !pageTitle ? seoTitle : `${pageTitle ?? seoTitle} | ${seoTitle}`;
  return {
    alternates: {
      en: new URL(buildLocalizedPath(path, "en"), nftSeoEntry.canonical).toString(),
      vi: new URL(buildLocalizedPath(path, "vi"), nftSeoEntry.canonical).toString(),
      xDefault: new URL(buildLocalizedPath(path, defaultLocale), nftSeoEntry.canonical).toString()
    },
    canonical: new URL(buildLocalizedPath(path, locale), nftSeoEntry.canonical).toString(),
    description,
    ...
    schemaTypes: nftSeoEntry.schemaTypes,
    title
  };
}
