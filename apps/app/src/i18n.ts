...
const appSeoEntry = loadSeoEntry("app.iai.one");
...
export function buildLocalizedPath(path: string, locale: Locale): string {
  const url = new URL(path, appSeoEntry.canonical);
...
export function getPageMetadata(path: string, locale: Locale, pageTitle?: string) {
  const seoTitle = locale === "vi" ? appSeoEntry.titleVi : appSeoEntry.titleEn;
  const description = locale === "vi" ? appSeoEntry.descriptionVi : appSeoEntry.descriptionEn;
  const title = path === "/" && !pageTitle ? seoTitle : `${pageTitle ?? seoTitle} | ${seoTitle}`;
  return {
    alternates: {
      en: new URL(buildLocalizedPath(path, "en"), appSeoEntry.canonical).toString(),
      vi: new URL(buildLocalizedPath(path, "vi"), appSeoEntry.canonical).toString(),
      xDefault: new URL(buildLocalizedPath(path, defaultLocale), appSeoEntry.canonical).toString()
    },
    canonical: new URL(buildLocalizedPath(path, locale), appSeoEntry.canonical).toString(),
    description,
    ...
    schemaTypes: appSeoEntry.schemaTypes,
    title
  };
}
